...
const Projects = () =>{
    ...
    const [tags, setTags] = useState(
        [...new Set(data.map(x => x.platform))].concat(
        [...new Set(data.map(x => x.studio))]).concat(
    ...
    const filter = (x) => {
        ...
        setPosts(data.filter(el => temp.includes(el.platform) && temp.includes(el.studio) && temp.includes(el.genre)));
    }
